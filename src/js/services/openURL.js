    ...
    if (platformInfo.isChromeApp) {
      ...
      gui.App.on('open', function(pathData) {
        if (pathData.indexOf('particl:') != -1) {
          $log.debug('Particl URL found');
          handleOpenURL({
            url: pathData.substring(pathData.indexOf('particl:'))
          });
      ...
      if (navigator.registerProtocolHandler) {
        ...
        navigator.registerProtocolHandler('web+particl', url, 'Particl Copay Wallet Handler');
        navigator.registerProtocolHandler('web+particl-copay', url, 'Particl Copay Wallet Handler');
        navigator.registerProtocolHandler('web+bitpay', url, 'Particl Wallet Handler');
      }
    }
  };
